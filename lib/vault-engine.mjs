...
export function validateGenerated(raw,sources) {
  ...
  const claims=raw.claims.slice(0,8).flatMap(c=>{if(typeof c.text!=='string'||!c.text.trim()||c.text.length>1600||!Array.isArray(c.citations)||!c.citations.length){rejected++;return [];}
    const valid=c.citations.length<=5&&c.citations.every(ref=>typeof ref.chunkId==='string'&&typeof ref.quote==='string'&&ref.quote.trim().length>=8&&byId.get(ref.chunkId)?.text.includes(ref.quote));
